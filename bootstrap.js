			...
			var savePngToDisk = function(size, refDeferred, blob) {
				...
				reader.onloadend = function() {
					...
					for (var i=0; i<sizeToName[size].length; i++) {
						if (i==0) {
							arrbufsArr.push(reader.result);
						} else {
							arrbufsArr.push(reader.result.slice());
						}
					}
					
					for (var i=0; i<sizeToName[size].length; i++) {
						var savePth = OS.Path.join(path_dirIconSet, sizeToName[size][i] + '.png');
						promiseAllArr_writePngs.push(tryOsFile_ifDirsNoExistMakeThenRetry('writeAtomic', [savePth, new Uint8Array(arrbufsArr[i]), {tmpPath:savePth+'.tmp'}], OS.Constants.Path.userApplicationDataDir, {causesNeutering:true}));
					}
...
function showPick4Badging(win) {
	...
	var blobCallback = function(path_to_save_at, refDeferred, blob) {
        ...
        reader.onloadend = function() {
			...
			var promise_writeArrView = tryOsFile_ifDirsNoExistMakeThenRetry('writeAtomic', [path_to_save_at, arrview, {tmpPath:path_to_save_at+'.tmp'}], fromPathBase, {causesNeutering:true});
