...
function returnIconset(aArg, aComm) {
	...
	var step0 = function() {
		...
		gBsComm.postMessage('setupFrameworker', aComm.CallbackTransferReturn({aId:fwId, aBootPort:port1, aFwPort:port2}, [port1, port2]), step1);
	};
	...
	var step1 = function(msgFrom_fsReturnIconset_onPageReady) {
		...
		// gBsComm.postMessage('destroyFrameworker', fwId);
		...
		for (var i=0; i<aBaseSrcImgPathArr.length; i++) {
			...
			gBsComm.postMessage('tellFrameworkerLoadImg', {aProvidedPath:aBaseSrcImgPathArr[i], aLoadPath:imgPathData[aBaseSrcImgPathArr[i]].img_src, aId:fwId}, tellFrameworkerLoadImgCallback.bind(null, aBaseSrcImgPathArr[i], deferred_loadImage));
		}
		...
		if (aOptions.aBadge) {
			for (var i=0; i<aOptions.aBadgeSrcImgPathArr.length; i++) {
				...
				var deferred_loadImage = new Deferred();
				promiseAllArr_loadImgAndGetImgDatas.push(deferred_loadImage.promise);
				gBsComm.postMessage('tellFrameworkerLoadImg', {aProvidedPath:aOptions.aBadgeSrcImgPathArr[i], aLoadPath:imgPathData[aOptions.aBadgeSrcImgPathArr[i]].img_src, aId:fwId}, tellFrameworkerLoadImgCallback.bind(null, aOptions.aBadgeSrcImgPathArr[i], deferred_loadImage));
			}
		}
	...
	var step4 = function() {
		...
		if (aOptions.saveScaledBadgeDir) {
			...
			for (var p in objOutputSizes) {
				...
				gBsComm.postMessage('tellFrameworkerDrawScaled', {aImgPath:objOutputSizes[p].badge.useKey, aDrawAtSize:objOutputSizes[p].badge.drawAtSize, aId:fwId}, tellFrameWorkerDrawScaledCb.bind(null, objOutputSizes, p, deferred_scaleBadge));
	...
	var step5 = function() {
		...
		for (var i=0; i<sortedDescP.length; i++) {
			...
			gBsComm.postMessage('tellFrameworker_dSoBoOOSb', {aImgPath:objOutputSizes[p].base.useKey, aDrawAtSize:objOutputSizes[p].base.drawAtSize, optBuf:getOptBuf, optOverlapObj:overlapObj, aId:fwId}, tellFrameworker_dSoBoOOSbCb.bind(null, objOutputSizes, p, deferred_scaleOutput));
	...
	var step7 = function() {
		...
		if (aOptions.dontMakeIconContainer) {
			gBsComm.postMessage('destroyFrameworker', fwId); // no more need for frameworker, destrooy it
			deferredMain_returnIconset.resolve({
				status: 'ok',
		...
		switch (aCreateType) {
			...
			case createTypeIco:
					...
					var step8_1 = function() {
						...
						gBsComm.postMessage('tellFrameworkerGetImgDatasOfFinals', {reqObj, aId:fwId}, function(aObjOfBufs) {
							gBsComm.postMessage('destroyFrameworker', fwId); // no more need for frameworker, destrooy it
